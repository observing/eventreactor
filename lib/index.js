(function EventReactorBootstrap(mount) {
  "use strict";
  ...
  var EventEmitter = mount.EventEmitter || process.EventEmitter;
   ...
   *
   * @constructor
   * @param {Boolean} manual manual initialize the override methods
   * @api public
   */

  function EventReactor(manual) {
    if (manual) return this;
  ...
  Object.keys(EventReactor.generation = {
    ...
    , 'either': function either() {
        ...
        args.push(handle);
        return this.every.apply(this, args);
      }
    ...
    , 'defer': new Function('event', [
        ...
        , 'process' in mount && 'nextTick' in mount.process
            ? 'process.nextTick('
            : ('setImmediate' in mount
               ? 'setImmediate('
               : 'setTimeout('
              )
    ...
    EventReactor.prototype[key] = function(proto, force) {
      proto = proto || EventReactor.prototype;

      // the every method already exists, and we don't want to override it
      if (key in proto && !force) return this;
  ...
  mount.EventReactor = EventReactor;
}(this)); // Douglas Crockford wants the goovy dog balls inside http://bit.ly/KUoK7g
